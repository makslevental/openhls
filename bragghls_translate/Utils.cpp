...
SmallVector<int64_t, 8> bragghls::getIntArrayAttrValue(Operation *op,
                                                       StringRef name) {
...
void bragghls::getMemAccessesMap(Block &block, MemAccessesMap &map,
...
bragghls::checkSameLevel(Operation *lhsOp, Operation *rhsOp) {
...
unsigned bragghls::getCommonSurroundingLoops(Operation *A, Operation *B,
                                             AffineLoopBand *band) {
...
bragghls::getBoundOfAffineMap(AffineMap map, ValueRange operands) {
...
bool bragghls::isFullyPartitioned(MemRefType memrefType) {
...
// returned as well.
int64_t bragghls::getPartitionFactors(MemRefType memrefType,
                                      SmallVector<int64_t, 8> *factors) {
...
unsigned bragghls::getChildLoopNum(Operation *op) {
  unsigned childNum = 0;
  for (auto &region : op->getRegions())
    for (auto &block : region)
      for (auto &op : block)
...
bool bragghls::getTileAndPointLoopBand(const AffineLoopBand &band,
...
AffineForOp bragghls::getLoopBandFromOutermost(AffineForOp forOp,
                                               AffineLoopBand &band) {
  band.clear();
  auto currentLoop = forOp;
  while (true) {
...
/// loops are collected.
void bragghls::getLoopBands(Block &block, AffineLoopBands &bands,
...
void bragghls::getArrays(Block &block, SmallVectorImpl<Value> &arrays,
                         bool allowArguments) {
...
Optional<unsigned> bragghls::getAverageTripCount(AffineForOp forOp) {
...
bool bragghls::checkDependence(Operation *A, Operation *B) {
...
void bragghls::localizeConstants(Block &block, int64_t bitsThreshold) {
...
FuncOp bragghls::getTopFunc(ModuleOp module, std::string topFuncName) {
  FuncOp topFunc;
...
FuncOp bragghls::getRuntimeFunc(ModuleOp module,
