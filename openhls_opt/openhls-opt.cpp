...
#define DEBUG_TYPE "openhls-naive-loop-unroll"
...
struct NaiveLoopUnroll : public PassWrapper<NaiveLoopUnroll, OperationPass<func::FuncOp>> {
  ...
  StringRef getArgument() const override { return "openhls-naive-loop-unroll"; }
...
void NaiveLoopUnroll::runOnOperation() {
  ...
  for (auto root : llvm::make_early_inc_range(outerLoops)) {
    ...
    for (AffineForOp forOp : llvm::reverse(nestedLoops)) {
      ...
      if (failed(openhls::loopUnrollUpToFactor(forOp, currentUnrollFactor, true)))
...
struct NaiveStoreLoadForward : public PassWrapper<NaiveStoreLoadForward, OperationPass<func::FuncOp>> {
  ...
  void runOnOperation() override;
  StringRef getArgument() const override { return "openhls-naive-store-load-forward"; }
  StringRef getDescription() const override {
...
int main(int argc, char **argv) {
  ...
  openhls::registerAllDialects(registry);
  PassRegistration<openhls::NaiveLoopUnroll>();
  PassRegistration<openhls::NaiveStoreLoadForward>();
  mlir::registerCSEPass();

  return mlir::failed(mlir::MlirOptMain(argc, argv, "OpenHLS Optimization Tool", registry, true));
}
